      ...
      <Container className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <Container>
          ...
          <Box className="grid gap-5">
            {experience.map((item) => (
              <Box
                className="bg-black hover:bg-white hover:text-black p-4 rounded-lg w-11/12 grid gap-4 cursor-pointer hover:scale-110 transition duration-300 ease-in"
                key={item}
              >
                <div className=" capitalize font-bold text-[#8750f7]">
                  {item.experience}
                </div>
          ...
          <Box className="grid gap-5">
            {education.map((item) => (
              ...
              >
                ...
                <div className="text-lg font-bold uppercase">{item.title}</div>
                <div className=" uppercase font-semibold">{item.institute}</div>
              </Box>
            ))}
          </Box>
        </Container>
